...
#' @param blastn path to the blastn executable. Currently, only version 2.2.31+ supported, since it's the only one with PROPER sam output
...
registerBlast = function(blastn,makeblastdb,samtools)
{
  ...
  blastEnv$blastn<<-blastn
  blastEnv$makeblastdb<<-makeblastdb
  blastEnv$samtools <<- samtools
...
addSides = function(chrom,pos,lengthSides,originalBuild)
{
  ...
  lengthSides = rep(lengthSides,length(pos))
  lengthSides[(pos+lengthSides)>chromLengths] = chromLengths[(pos+lengthSides)>chromLengths] - pos
  lengthSides
}
...
doAlignment = function(seqs,ref,tmpdir,aligner,memlimit,editDistance,blastArgs = "")
{
  if(aligner == "Rbowtie")
  {
  ...
  } else if(aligner =="Rsubread")
  {
    ...
    Rsubread::align(index =ref,readfile1 = sprintf("%s",seqs),type = 1,phredOffset = 64,maxMismatches = editDistance,complexIndels=T,indels=200,minFragLength = 25)
  ...
  } else if(aligner == "blastn")
  {
    ...
    system(sprintf("%s -in %s -out %s -dbtype nucl -parse_seqids",blastEnv$makeblastdb,ref,blastdb))
    print("performing alignment (may take a long time, this IS blast)....")
    system(sprintf("%s -db %s -query %s %s -out %s",blastEnv$blastn,blastdb,seqs,blastArgs,sampath),ignore.stderr = T,ignore.stdout = T)
    system(sprintf("%s view -bS %s -o %s",blastEnv$samtools,sampath,bampath),ignore.stderr = T,ignore.stdout = T)
    return(bampath)
    }
}
...
#' @param editDistance Maximum allowed edit distance (as determined by the NM tag in the alignment SAM file) between the synthetic reads generated by the package and the new build. Defaults to 0 which is perfect matches.
...
#' @export
liftover = function(chrom,start,end = NULL,originalBuild,newBuild,varnames=NULL,lengthSides=25,editDistance = 0,tmpdir = NULL,aligner = "Rsubread",memlimit = 8000, maxAllowedLength = 500,blastArgs= "-max_hsps 1 -parse_deflines -outfmt 15 -max_target_seqs 1")
{
  if(aligner == "Rsubread")
  {
  ...
  aligned = doAlignment(seqs = readsPath,ref = newBuild ,tmpdir = tmpdir,aligner = aligner,memlimit = memlimit,editDistance = editDistance,blastArgs)
  ...
  if(aligner == "Rbowtie")
  {
    choos = intersect(which(algn[[1]]$tag$NM<=editDistance),choos)
  }
